...
//// ------------------------------------------------------------
  /// DSEM code update w new code from DSEM main branch 12/19/24
  ...
  vector<Type> tmp2=slctfsh.row(y1-1);
  slctfsh_logit=-log(1/(tmp2 -1e-10)-1);
  ...
  RMSE_srv4=0;
    ...
    // not necessary with dsem in the assessment mode
    ...
    objfun = -sum(loglik);
    objfun +=jnll; // add DSEM likelihood
